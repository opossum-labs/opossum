...
pub fn cambox_2w() -> OpmResult<NodeGroup> {
    let _config = RectangleConfig::new(millimeter!(11.33), millimeter!(7.13), millimeter!(0., 0.))?;
    //let cam_aperture = Aperture::BinaryRectangle(config);

    let mut cb = NodeGroup::new("CamBox 2w");

    let d1 = cb.add_node(Dummy::new("d1"))?;
    let bs1 = cb.add_node(BeamSplitter::new(
        "bs1",
    ...
    // FF path
    let bs_ff = cb.add_node(BeamSplitter::new(
        "bs_ff",
    ...
    let nf_bs = cb.add_node(BeamSplitter::new(
        "nf bs",
        &SplittingConfigBuilder::FixedRatio(0.5),
    )?)?;

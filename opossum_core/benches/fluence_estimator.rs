...
fn criterion_kde(c: &mut Criterion) -> OpmResult<()> {
    let distribution = Hexapolar::new(millimeter!(50.0), 3)?;
...
fn criterion_binning(c: &mut Criterion) -> OpmResult<()> {
    let distribution = SobolDist::new(millimeter!(50.0), millimeter!(50.0), 10000)?;
    let points = distribution.generate();
    let weight = joule!(1.0) / Ratio::new::<uom::si::ratio::ratio>(points.len() as f64);
    let mut hit_map = RaysHitMap::default();
    for p in points {

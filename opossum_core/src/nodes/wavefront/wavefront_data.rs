    ...
    pub fn from_rays(
        ...
        if center_wavelength_flag {
            ...
            if average_flag {
                ...
                let closest_wvl = wvls.clone().into_iter().fold(wvls[0], |a, b| {
...
impl WaveFrontMap {
    ...
    /// Returns the wavelength of this [`WaveFrontErrorMap`].
    #[must_use]
...
mod test {
    ...
    fn wavefront_map_from_rays() -> OpmResult<()> {
        ...
        for (i, val) in wf_map.wf_map.into_iter().enumerate() {
            if i != 0 {
                assert_relative_eq!(
                    val,
                    &(10000. * (1. - f64::sqrt(2.))),

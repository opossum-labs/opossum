...
impl_validator!(IsNormal, |_self, v: &Point2<f64>| v.x.is_normal() && v.y.is_normal(), Point2<f64>);
impl_validator!(IsNormal, |_self, v: &Point2<Length>| v.x.is_normal() && v.y.is_normal(), Point2<Length>);
...
mod tests {
    ...
    fn test_is_normal_point2_length() {
        ...
        let p_invalid_subnormal = Point2::new(Length::new::<meter>(f64::MIN_POSITIVE / 2.0), Length::new::<meter>(1.0));
...
fn test_is_normal_angle() {
    ...
    assert!(validator.validate(&Angle::new::<radian>(f64::MIN_POSITIVE)).is_ok());

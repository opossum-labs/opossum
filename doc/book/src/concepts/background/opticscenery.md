...
In addition, it should implement the top-most function to export the graph to the graphwiz format for easy visualization. Last but not least it also contains the top-level `analyze` function starting the actual analysis process. The type of analysis has to be given here making use of the well-known strategy pattern.

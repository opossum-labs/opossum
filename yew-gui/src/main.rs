...
// use web_sys::{js_sys::{self, Promise}, DragEvent, HtmlElement};
// use yew::{function_component, html, use_node_ref, use_state, Callback, Html, TargetCast, UseStateHandle};
// use yew_gui::node_graph::node_element::{Connection, Node};
// use uuid::Uuid;

// #[wasm_bindgen(module = "/commands.js")]
// extern "C" {
//     // #[wasm_bindgen(js_name = addNode, catch)]
//     pub fn addNode(node_type: String) -> js_sys::Promise;
//     pub async fn printBeepBoop() -> js_sys::Promise;
// }
...
//     let nodes = use_state(|| Vec::<(Uuid, i32, i32, String, bool)>::new());
//     let connections = use_state(|| HashMap::<Uuid, Connection>::new()); // Verbindungen
//     let selected_port = use_state(|| None::<(Uuid, String)>); // Aktuell ausgewählter Port (Node-ID, Port-Typ)
//     let active_node = use_state(|| None::<(Uuid, String)>); // Aktuell ausgewählter Port (Node-ID, Port-Typ)
...
//             if let Some(div) = log_container_ref.cast::<HtmlElement>() {
...
//     let on_add_node = create_add_node_handler(nodes.clone(), add_log.clone());

//     let on_node_double_click = {
//         let active_node = active_node.clone();
//         Callback::from(move |(id, name): (Uuid, String)| {
...
//             </div>
//             <div>
//             <div ref={log_container_ref} class="log-container">
...
//     nodes: UseStateHandle<Vec<(Uuid, i32, i32, String, bool)>>,
...
//         let new_x = 50 + (nodes.len() as i32 % 20) * 100;
//         let new_y = 50 + (nodes.len() as i32 / 20) * 100;

//         let mut new_nodes = (*nodes).clone();
        
//         spawn_local(async move {
//             let result = printBeepBoop().await;
//             let result = wasm_bindgen_futures::JsFuture::from(result).await;
//             add_log_handler.emit("fickscheiße".into());
//             add_log_handler.emit(format!("{}", result.clone().unwrap().as_string().unwrap()));
//         });
//         // spawn_local(async move {
...
//         //     match result {
//         //         Ok(new_id) => {
//         //             add_log_handler.emit(format!("test{}", new_id.as_string().unwrap()));

//         //             // if let Ok(new_uuid) = serde_json::from_str(new_id.as_string().unwrap().as_str()) {
//         //             //     new_nodes.push((new_uuid, new_x, new_y, name.clone(), is_source.clone())); // Neue Position für die Node
//         //             //     nodes.set(new_nodes); // Aktualisiere den Zustand
//         //             //     add_log_handler.emit(format!("Added node: {}", name));
//         //             // } else {
//         //             //     add_log_handler.emit(format!("Error while adding node \"{}\" due to uuid deserialization failure", name));
//         //             // }
//         //         }
//         //         Err(e) => {
//         //             // add_log_handler.emit(format!("Error while adding node \"{}\". Error: {}", name, e.as_string().unwrap()));
//         //             add_log_handler.emit(format!("asd{}",  e.as_string().unwrap()));
//         //         }
//         //         _ => {
...
//     port: UseStateHandle<Option<(Uuid, String)>>,
//     connections: UseStateHandle<HashMap<Uuid, Connection>>,
...
use wasm_bindgen_futures::spawn_local;
use web_sys::{console::log, js_sys};
use yew::prelude::*;

#[wasm_bindgen(module = "/invoke.js")]
extern "C" {
    pub fn invokeIncrementCounter(value: u32) -> js_sys::Promise;
}
#[function_component(App)]
fn app() -> Html {
    let counter = use_state(|| 0);
    let increment_value = use_state(|| 1); // Wert für das Inkrement

    let increment = {
        let counter = counter.clone();
        let increment_value = increment_value.clone();
        Callback::from(move |_| {
            let counter = counter.clone();
            let increment_value = increment_value.clone();
            spawn_local(async move {
                let value = *increment_value;
                let result = invokeIncrementCounter(value);
                let result = wasm_bindgen_futures::JsFuture::from(result).await;

                match result {
                    Ok(new_counter) => {
                        counter.set(new_counter.as_f64().unwrap_or(0.0) as u32);
                    }
                    Err(error) => {
                        log!("Fehler: {:?}", error);
                    }
    ...
    html! {
        <div>
            ...
            <p>{ format!("Inkrementwert: {}", *increment_value) }</p>
            <button onclick={increment}>{"Inkrementieren"}</button>
            <button onclick={Callback::from(move |_| increment_value.set(*increment_value + 1))}>{"Inkrementwert erhöhen"}</button>
        </div>
    }
}

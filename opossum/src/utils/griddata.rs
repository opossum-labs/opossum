...
use crate::{error::{OpmResult, OpossumError}, plottable::AxLims};

#[must_use]
pub fn calc_closed_poly_area(poly_coords: &[Point2<f64>]) -> f64 {
    let mut area = 0.;
...
pub fn interpolate_2d_scatter_data(
    ...
    let voronoi_opt = create_voronoi_cells(&MatrixXx2::from_columns(&[scattered_data.column(0), scattered_data.column(1)]), &x_bounds, &y_bounds);

    if let Some(voronoi) = voronoi_opt{
    Ok(interpolate_2d_triangulated_scatter_data(
        &voronoi,
        &scattered_data.column(2),
        x_interp,
        y_interp,
    ))
}
else{
    Err(OpossumError::Other("Could not create Voronoi diagram! Interpolation not possible".into()))
}
}

pub fn meshgrid(x: &DVector<f64>, y: &DVector<f64>) -> (DMatrix<f64>, DMatrix<f64>) {
    let x_len = x.len();
    let y_len = y.len();
...
pub fn linspace(start: f64, end: f64, num: f64) -> OpmResult<DVector<f64>> {
    if let Some(num_usize) = num.to_usize() {
...
pub fn interpolate_2d_triangulated_scatter_data(
    voronoi: &VoronoiDiagram<VPoint>,
    z_data: &DVectorSlice<f64>,
    x_interp: &DVector<f64>,
    y_interp: &DVector<f64>,
) -> (DMatrix<f64>, DMatrix<f64>) {
    let num_axes_points_x = x_interp.len();
    let num_axes_points_y = y_interp.len();
    let mut interp_data = DMatrix::<f64>::from_element(num_axes_points_x, num_axes_points_y, f64::NAN);
    let tri_index_mat = Matrix3xX::from_vec(voronoi.delaunay.triangles.clone()).transpose();
    ...
    let x_bin = x_interp[1] - x_interp[0];
    let y_bin = y_interp[1] - y_interp[0];
    ...
    for tri_idxs in tri_index_mat.row_iter() {
        ...
        let x_i = (x_min - x_interp[0]) / x_bin;
        let x_f = (x_max - x_interp[0]) / x_bin;
        let y_i = (y_min - y_interp[0]) / y_bin;
        let y_f = (y_max - y_interp[0]) / y_bin;

...
use crate::{ray::Ray, refractive_index::RefractiveIndexType, surface::{GeoSurf, GeoSurface}, utils::geom_transformation::Isometry};
...
pub trait SDF {
    ...
    fn sdf_union_point<'a>(&self, sdf_vec: &[&GeoSurf], p: &Point3<f64>) -> f64 {
        if sdf_vec.is_empty() {
    ...
    fn sdf_intersection_vec_of_points<'a>(
        &self,
        sdf_vec: &[&GeoSurf],
        p_vec: &Vec<Point3<f64>>,
    ) -> Vec<f64> {
    ...
    fn sdf_intersection_point<'a>(&self, sdf_vec: &[&GeoSurf], p: &Point3<f64>) -> f64 {
        if sdf_vec.is_empty() {
            self.sdf_eval_point(p)
        } else {
            sdf_vec.iter().fold(self.sdf_eval_point(p), |arg0, sdf| {
    ...
    fn sdf_subtraction_vec_of_points<'a>(
        &self,
        sdf_vec: &[&GeoSurf],
        p_vec: &Vec<Point3<f64>>,
    ) -> Vec<f64> {
    ...
    fn sdf_subtraction_point<'a>(&self, sdf_vec: &[&GeoSurf], p: &Point3<f64>) -> f64 {
        if sdf_vec.is_empty() {
            self.sdf_eval_point(p)
        } else {
            sdf_vec.iter().fold(self.sdf_eval_point(p), |arg0, sdf| {

...
fn main() -> OpmResult<()> {
    ...
    let circle = CircleConfig::new(millimeter!(9.9), millimeter!(0., 0.))?;
    lens1.set_input_aperture("front", &Aperture::BinaryCircle(circle))?;
    let i_pl1 = scenery.add_node(lens1);
    let i_pr1 = scenery.add_node(Propagation::new("100 mm", millimeter!(100.0))?);
    let i_pr2 = scenery.add_node(Propagation::new("50 mm", millimeter!(50.0))?);
    let mut lens2 = ParaxialSurface::new("50 mm lens", millimeter!(50.0))?;
    let circle = CircleConfig::new(millimeter!(3.5), millimeter!(0., 0.))?;

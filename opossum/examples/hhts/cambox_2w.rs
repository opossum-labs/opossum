...
pub fn cambox_2w() -> OpmResult<NodeGroup> {
    ...
    // NF path
    let nf_d1 = cb.add_node(Propagation::new("nf_d1", millimeter!(35.0))?)?;
    let nf_lens1 = cb.add_node(ParaxialSurface::new("NF lens1", millimeter!(125.0))?)?;
    let nf_d2 = cb.add_node(Propagation::new("nf_d1", millimeter!(250.0))?)?;
    let nf_lens2 = cb.add_node(ParaxialSurface::new("NF lens2", millimeter!(125.0))?)?;
    let nf_d3 = cb.add_node(Propagation::new("nf_d1", millimeter!(50.0))?)?;
    let nf_bs = cb.add_node(BeamSplitter::new("nf bs", &SplittingConfig::Ratio(0.5))?)?;
    let nf_d4 = cb.add_node(Propagation::new("nf_d4", millimeter!(130.0))?)?;

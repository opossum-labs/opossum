...
use opossum::{
    centimeter, degree, energy_distributions::UniformDist, error::OpmResult, joule, lightdata::LightData, millimeter, nanometer, nodes::{
        round_collimated_ray_source, Lens, NodeReference, RayPropagationVisualizer, ReflectiveGrating, Source, ThinMirror
    }, num_per_mm, optical::{Alignable, Optical}, position_distributions::Hexapolar, rays::Rays, refractive_index::RefrIndexSellmeier1, spectral_distribution::Gaussian, utils::geom_transformation::Isometry, OpticScenery
};
use std::path::Path;
...
fn main() -> OpmResult<()> {
    ...
    let mut rays = Rays::new_collimated_with_spectrum(
        &Gaussian::new((nanometer!(1040.), nanometer!(1068.)), 50, nanometer!(1054.), nanometer!(8.), 1.)?,
    ...
    let mir_1 = scenery.add_node(ThinMirror::new("mirr"));
    ...
    let i_prop_vis = scenery.add_node(RayPropagationVisualizer::new("Ray_positions", Some(Vector3::x())));


    scenery.connect_nodes(i_src, "out1", i_g1, "input", millimeter!(400.0))?;
    scenery.connect_nodes(i_g1, "diffracted", lens1, "front", millimeter!(100.0))?;
    scenery.connect_nodes(lens1, "rear", mir_1, "input", millimeter!(100.0))?;
    scenery.connect_nodes(mir_1, "reflected", i_prop_vis, "in1", millimeter!(400.0))?;

    // scenery.connect_nodes(mir_1, "reflected", lens_1_ref, "rear", millimeter!(1500.0))?;
    // scenery.connect_nodes(lens_1_ref, "front", g1ref, "diffracted", millimeter!(1500.0))?;
    ...
    scenery.save_to_file(Path::new("./opossum/playground/martinez_test.opm"))?;
    Ok(())
}

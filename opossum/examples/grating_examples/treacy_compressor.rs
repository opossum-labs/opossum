use opossum::{
    ...
    num_per_mm, optic_node::{Alignable, OpticNode},
};
...
pub fn treacy_compressor(alignment_wvl: Length) -> OpmResult<NodeGroup> {
    ...
    let mut rot = grating_1.node_attr().alignment().as_ref().unwrap().rotation();
    // rot.z = degree!(0.01);
    ...
    let mut rot = grating_4.node_attr().alignment().as_ref().unwrap().rotation();
    // rot.z = degree!(0.01);

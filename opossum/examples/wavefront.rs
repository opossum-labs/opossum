...
fn main() -> OpmResult<()> {
    ...
    let i_l = scenery.add_node(ParaxialSurface::new("lens", Length::new::<meter>(1e-3))?);
    let i_p2 = scenery.add_node(Propagation::new("propagation", Length::new::<meter>(0.5))?);
    let i_wf2 = scenery.add_node(WaveFront::default());

use nalgebra::{MatrixXx1, iter::MatrixIter, MatrixSlice1, MatrixSliceXx1, MatrixSlice, vector, VectorN, U5, DVectorSlice, Dynamic, Const, SVector, DVector};
use std::{time::{Duration, Instant}, ops::Add};
...
fn pairwise_sum_matrix(input: &DVectorSlice<f64>) -> f64 {
    ...
    if vec_len < 64{
        ...
        let new_size = vec_len/2;
        pairwise_sum_matrix(&input.rows(0, new_size)) + pairwise_sum_matrix(&input.rows(new_size, vec_len-new_size))
    }
}
...
fn main() {
    ...
    let mat_wo_inf = MatrixXx1::from(mat22.iter().cloned().filter(|x| !x.is_nan() & x.is_finite()).collect::<Vec<f64>>());
    ...
    println!("Time elapsed in kahan matrix summation() is: {:?}", duration);

    let start = Instant::now();
    let energy_vec1 = rays_vec2.iter().cloned().tree_fold1(|a,b| a+b);//sum_with_accumulator::<NaiveSum<f64>>();
    let duration = start.elapsed();
    println!("Time elapsed in pariwise itertools summation() is: {:?}", duration);
    ...
    let duration = start.elapsed();
    println!("Time elapsed in pairwise self written summation() is: {:?}", duration);

    let start = Instant::now();
    let energy_vec4 = rays_vec2.sum_with_accumulator::<Sum2<f64>>();
    let duration = start.elapsed();
    println!("Time elapsed in pairwise matrix self written summation() is: {:?}", duration);

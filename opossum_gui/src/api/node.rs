...
pub async fn post_paste_nodes(
    group_id: Uuid,
    pos: Point2D<f64>,
) -> Result<(HashMap<Uuid, Vec<NodeInfo>>, Vec<AnalyzerInfo>, HashMap<Uuid, Vec<ConnectInfo>>), String> {
    HTTP_API_CLIENT()
        .post::<(Uuid, (f64, f64)), (HashMap<Uuid, Vec<NodeInfo>>, Vec<AnalyzerInfo>, HashMap<Uuid, Vec<ConnectInfo>>)>(

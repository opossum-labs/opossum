    ...
    use_effect(move || {
        ...
        if let Some(cmd) = &*(cxt_command_val) {
            match cmd {
                ...
                CxtCommand::JumpToMappedPort { mapped_node_id, parent } => {node_editor_command_handler.call(Some(NodeEditorCommand::JumpToMappedPort {mapped_node_id: *mapped_node_id, parent: parent.clone()}));}
            }
        }
    });

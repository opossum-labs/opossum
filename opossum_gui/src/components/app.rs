    ...
    use_effect(move || {
        ...
        if let Some(cmd) = &*(cxt_command_val) {
            match cmd {
                ...
                CxtCommand::MapNodePort { port_type, group_port_name, mapped_node_port_name, mapped_node_id, group_id } => {
                    node_editor_command_handler.call(Some(NodeEditorCommand::MapNodePort { port_type: *port_type, group_port_name: group_port_name.clone(), mapped_node_port_name: mapped_node_port_name.clone(), mapped_node_id: *mapped_node_id, group_id: *group_id}))
                    // { node_id:*node_id, port_name:port_name.clone(), port_type:*port_type, graph_id:*graph_id }
            }}
        }
    });

use crate::{OPOSSUM_UI_LOGS, components::scenery_editor::{
    GraphState,
...
impl WorkspaceHandlers {
    ...
    pub fn add_port_map(&self, group_id: Uuid, group_port_name: String, mapped_node_port_name: String, mapped_node_id: Uuid){
        self.add_port_map.call(((group_id, mapped_node_id), (group_port_name, mapped_node_port_name)));
    }
...
fn add_port_map_handler(mut workspace: Signal<GraphsWorkspaceState>) 
    -> EventHandler<((Uuid,Uuid), (String, String))>{
        EventHandler::new(move |((group_id, mapped_node_id), (group_port_name, mapped_node_port_name)): ((Uuid,Uuid), (String, String))| {
            let ws = workspace.write();

            if let Some(mut graph_store)= ws.get_graph_store(group_id){
                if let Err(e) = graph_store.write().mapped_ports.write().add(&group_port_name, mapped_node_id, &mapped_node_port_name){

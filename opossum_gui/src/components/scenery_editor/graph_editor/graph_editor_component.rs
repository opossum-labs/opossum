...
impl GraphsWorkspaceState {
    fn get_graph_state(&self, graph_id: Uuid) -> Option<Signal<GraphState>> {
    ...
    fn get_editor_state(&self, graph_id: Uuid) -> Option<Signal<EditorState>> {
        self.tabs
            .read()
            .get(&graph_id)
            .map(|g| g.read().editor_state.into())
    }
    fn get_active_editor(&self) -> Option<Signal<EditorState>> {
    ...
    fn get_graph_nodes(&self, graph_id: Uuid) -> Option<Signal<HashMap<Uuid, NodeElement>>> {
        self.tabs
            .read()
            .get(&graph_id)
            .map(|g| g.read().graph_store.read().nodes().into())
    }

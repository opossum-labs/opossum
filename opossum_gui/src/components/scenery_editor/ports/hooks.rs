use dioxus::{html::{MouseEvent, PointerInteraction, geometry::euclid::default::Point2D, input_data::MouseButton}, prelude::*};
use opossum_core::prelude::PortType;
use uuid::Uuid;

use crate::{CONTEXT_MENU, components::{context_menu::cx_menu::{CxMenu, CxtCommand}, scenery_editor::{EditorState, GraphStore, GraphsWorkspaceState, edges::edges_component::{EdgePort, NewEdgeCreationStart}, graph_editor::DragStatus}}};

pub fn use_on_mouse_down(mut workspace: Signal<GraphsWorkspaceState>, node_id: Uuid, port_name: String, port_type: PortType, abs_port_position: Point2D<f64> ) -> EventHandler<MouseEvent> {
...
pub fn use_on_mouse_leave(mut editor_status: Signal<EditorState>) -> EventHandler<MouseEvent>{
    EventHandler::new(move |event: MouseEvent| {
...
pub fn use_on_mouse_enter (mut editor_status: Signal<EditorState>, port_name: &String, node_id: Uuid, port_type: PortType, is_mapped_port: bool) -> EventHandler<MouseEvent>{
    EventHandler::new({
...
pub fn use_on_context_menu(workspace: ReadSignal<GraphsWorkspaceState>, graph_store: ReadSignal<GraphStore>, node_id: Uuid,
port_name: String, port_type: PortType)-> EventHandler<MouseEvent>{
    EventHandler::new(move |event: MouseEvent|{

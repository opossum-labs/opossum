    ...
    fn setter_from_obj(&self) -> impl FnMut(&mut FibonacciRectangle, f64) {
        match self {
            Self::SideLengthX => move |obj: &mut FibonacciRectangle, val: f64| {
                obj.set_side_length_x(millimeter!(val)).log_err_with_context("`set_side_length_x` of fibonacci-rectangle.");
            },
            Self::SideLengthY => move |obj: &mut FibonacciRectangle, val: f64| {
                obj.set_side_length_y(millimeter!(val)).log_err_with_context("`set_side_length_y` of fibonacci-rectangle.");
            },
            Self::Points => move |obj: &mut FibonacciRectangle, val: f64| {
                if let Some(val) = try_f64_to_usize(val){
                    obj.set_nr_of_points(val).log_err_with_context("`set_nr_of_points` of fibonacci-rectangle.");
                }
            },
        }
    }

...
pub fn NodeSpectralDistInputs(
    ...
    let dist_params =
        get_spectral_dist_input_params(spectral_dist_type.clone(), light_data_builder_sig);
                            ...
                            if let (Some(wvl_val), Some(rel_int_val)) = (wvl_opt, rel_int_opt) {
                                if let (Ok(wvl), Ok(rel_int)) = (
                                    ...
                                    OPOSSUM_UI_LOGS
                                        ...
                                        .add_log(
                                            format!(
                                                "Could not parse laser line inputs! Wavelength: {:?}. Relative Intensity: {:?}",
                                                wvl_opt,
                                                rel_int_opt,
                                            )
                                                .as_str(),
                                ...
                                OPOSSUM_UI_LOGS
                                    ...
                                    .add_log(
                                        format!(
                                            "Wrong input inputs for adding laser line! Wavelength: {:?}. Relative Intensity: {:?}",
                                            wvl_opt,
                                            rel_int_opt,
                                        )
                                            .as_str(),
